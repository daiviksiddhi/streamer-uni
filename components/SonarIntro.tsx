...
function RadarScene() {
  ...
  const lastHoverCell = useRef<GridCell | null>(null);
  ...
  function updateHoverCells(x: number, y: number) {
    const nextCell = {
      column: Math.floor(x / gridSize),
      row: Math.floor(y / gridSize)
    };
    const previousCell = lastHoverCell.current;
    ...
    lastHoverCell.current = nextCell;

    const columnDistance = nextCell.column - (previousCell?.column ?? nextCell.column);
    const rowDistance = nextCell.row - (previousCell?.row ?? nextCell.row);
    const steps = Math.max(Math.abs(columnDistance), Math.abs(rowDistance), 1);
    const newCells = new Map<string, HoverCell>();

    for (let step = previousCell ? 1 : 0; step <= steps; step += 1) {
      const column = Math.round(
        (previousCell?.column ?? nextCell.column) + (columnDistance * step) / steps
      );
      const row = Math.round(
        (previousCell?.row ?? nextCell.row) + (rowDistance * step) / steps
      );
      const id = hoverCellId.current;
      hoverCellId.current += 1;
      newCells.set(`${column}-${row}`, {
        id,
        x: column * gridSize,
        y: row * gridSize
      });
    ...
    const refreshedKeys = new Set(newCells.keys());
    setHoverCells((cells) =>
      [
        ...cells.filter(
          (cell) =>
            !refreshedKeys.has(`${cell.x / gridSize}-${cell.y / gridSize}`)
        ),
        ...newCells.values()
      ].slice(-maxHoverCells)
    );
  ...
  function handlePointerMove(event: PointerEvent<HTMLElement>) {
    ...
    pointerFrame.current = requestAnimationFrame(() => {
      ...
      if (pointer) {
        updateHoverCells(pointer.x, pointer.y);
      }
    });
  }
    ...
    >
      ...
      {hoverCells.map((cell) => (
        <span
          ...
          onAnimationEnd={() => {
            setHoverCells((cells) => cells.filter(({ id }) => id !== cell.id));
          }}
        />
      ))}
